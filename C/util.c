...
void compute_forces(double dt){
    ...
    /* central force — px/py/pz locals, Gm[], r_sq reuse */
    for (i = 0; i < Nbody; i++) {
        double px = pos[0][i];
        double py = pos[1][i];
        double pz = pos[2][i];
        ...
        r_val = sqrt(r_sq);
        double gm = Gm[i] * M_central / (r_sq * r_val);
        f[0][i] -= gm * px;
        f[1][i] -= gm * py;
        f[2][i] -= gm * pz;
    }

    /* pairwise forces — cache blocked + branchless + local accumulator */
    for (i = 0; i < Nbody; i++) {
        double gmi = Gm[i];
        double ri  = radius[i];
        fix = 0.0; fiy = 0.0; fiz = 0.0;
        local_col = 0;
        for (jb = i+1; jb < Nbody; jb += BLOCK) {
            jend = (jb + BLOCK < Nbody) ? jb + BLOCK : Nbody;
            for (j = jb; j < jend; j++) {
                dx = pos[0][i] - pos[0][j];
                dy = pos[1][i] - pos[1][j];
                dz = pos[2][i] - pos[2][j];
